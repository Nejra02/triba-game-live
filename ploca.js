...
krugovi.addEventListener('click', function(event) {
    ...
    const rect = krugovi.getBoundingClientRect();
    const xklik = (event.clientX - rect.left) * krugovi.width / rect.width;
    const yklik = (event.clientY - rect.top) * krugovi.height / rect.height;
        ...
        if (udaljenost <= krug.radius * 1.4) {
            ...
            if(validan && !sijecese && !sijeceKrug){
                ...
                if(brojac >= 2 && !sijecese && !sijeceKrug){
                    ...
                    if (brojac === 2 && !imaMogucZavrsetak()) {
                        // Trenutni igrac ne moze dovrsiti potez; prethodni je pobjednik.
                        zavrsiIgru(igraPrvi ? 2 : 1);
                        return;
                    }
                }
            }
    ...
    if(brojac === 3){
        ...
        if(!sijecese && !sijeceKrug){
            ...
            trokut = [];
            brojac=0;
            const pobjednik = igraPrvi ? 1 : 2;
            promijeniIgraca();
            postaviLinije();
            provjeriKraj(pobjednik);
        }
        else{
...
// Provjeravamo stvarni moguci trougao, ne samo broj slobodnih linija.
function imaMogucTrokut(pocetneTacke = []) {
    const slobodni = lista.filter(k => k.color === 'grey' && k.blocked !== true);
    const kandidati = pocetneTacke.length ? pocetneTacke : slobodni;
    for (const a of kandidati) {
        for (const b of slobodni) {
            if (a === b || !validnaNovaDuz(a, b)) continue;
            for (const c of slobodni) {
                if (c === a || c === b || orijentacija(a,b,c) === 0) continue;
                if (!validnaNovaDuz(b,c) || !validnaNovaDuz(c,a)) continue;
                // Nijedna nova duz ne smije proci kroz treci vrh.
                if (presijecaKrug(a,b,c) || presijecaKrug(b,c,a) || presijecaKrug(c,a,b)) continue;
                return true;
            }
        }
    }
...
function imaMogucZavrsetak() {
    if (trokut.length !== 2) return true;
    const [a,b] = trokut;
    return lista.some(c => c !== a && c !== b && c.color === 'grey' && c.blocked !== true &&
        orijentacija(a,b,c) !== 0 && validnaNovaDuz(b,c) && validnaNovaDuz(c,a) &&
        !presijecaKrug(a,b,c) && !presijecaKrug(b,c,a) && !presijecaKrug(c,a,b));
}
function validnaNovaDuz(a,b) {
    return !provjeriPresjek([a,b],listalinija) && !prolaziKrozKrug([a,b]);
}
function zavrsiIgru(pobjednik) {
    if (igraZavrsena) return;
    igraZavrsena = true;
    alert(`Pobijedio je igrač ${pobjednik}!`);
...
function provjeriKraj(pobjednik) {
    if (!igraZavrsena && !imaMogucTrokut()) zavrsiIgru(pobjednik);
}
